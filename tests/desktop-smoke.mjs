...
function staticAudit(){
  ...
  const editorialCss=fs.readFileSync('site-editorial-v49.css','utf8');
  const css=bridgeCss+'\n'+luxeCss+'\n'+atelierCss+'\n'+cinematicCss+'\n'+editorialCss;
  ...
  const report={
    ...
    hasSmoothMotion:/FG_SITE_NAV_V6/.test(nav)&&/view-transition/.test(nav)&&/IntersectionObserver/.test(nav),
    ...
    editorialTheme:/installEditorialTheme/.test(nav)&&/site-editorial-v49\.css/.test(nav),
    ...
    editorialCss:/FloodGuard Editorial Story v49/.test(editorialCss)&&bridgeCss.includes('site-editorial-v49.css')&&/fg49section/.test(editorialCss)&&/repeat\(12/.test(editorialCss),
  ...
  if(!report.editorialTheme)throw new Error('Editorial v49 theme loader missing');
  ...
  if(!report.editorialCss)throw new Error('Editorial Story v49 design system is incomplete');
...
async function run(){
  ...
  if(await home.locator('link[href*="site-editorial-v49.css"]').count()<1)throw new Error('Editorial stylesheet missing on homepage');
  if((await home.locator('html').getAttribute('data-fg-cinematic'))!=='1')throw new Error('Cinematic mode not enabled on homepage');
  if((await home.locator('html').getAttribute('data-fg-editorial'))!=='1')throw new Error('Editorial mode not enabled on homepage');
  ...
  if(await home.locator('#fgMultiPageExplore .fgmp-card').count()<7)throw new Error('Homepage showcase incomplete');
  const firstCard=home.locator('#fgMultiPageExplore .fgmp-card').first();
  const firstRadius=await firstCard.evaluate(el=>getComputedStyle(el).borderRadius);if(!/^8px|7px$/.test(firstRadius))throw new Error('Editorial showcase styling not applied: '+firstRadius);
  ...
  for(const [path,selector,label] of layouts){
    ...
    if(await p.locator('link[href*="site-editorial-v49.css"]').count()<1)throw new Error(path+' editorial stylesheet missing');
  ...
  await login.waitForTimeout(650);if(!await login.locator('#loginEmail').isVisible().catch(()=>false))throw new Error('login form hidden');
